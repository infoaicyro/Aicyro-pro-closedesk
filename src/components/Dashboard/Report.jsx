      ...
      if (data) {
        const liveData = Object.keys(data).map((key) => {
          ...
          return {
            id: key,
            ...item,
            // FIX: Map the correct database fields to the 'timestamp' property your UI expects
            timestamp:
              item.conversation_started_at || item.last_interaction_at || null,
          };
        });

        setLeads(
          liveData
            .filter((lead) => lead.timestamp) // Safety check to prevent NaN crashes
            .sort(
              (a, b) =>
