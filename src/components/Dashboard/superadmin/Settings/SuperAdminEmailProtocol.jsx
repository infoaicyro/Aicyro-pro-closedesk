...
export default function SuperAdminEmailProtocol() {
  ...
  useEffect(() => {
    ...
    // 🔥 Listen for messages posted by the OAuth popup window
  ...
  // 🔥 Trigger to open the OAuth flow in a centered popup window
  ...
  const activeProviderName = savedProvider === "google_oauth" ? "Google" : savedProvider === "microsoft" ? "Microsoft" : savedProvider === "custom" ? "Custom SMTP" : "";
  ...
  return (
    <div className="max-w-[800px] mx-auto p-4 sm:p-8 lg:p-12 animate-acy-fade font-sans bg-[#F9FAFB] min-h-screen relative">
      <div className="bg-white rounded-[24px] p-6 sm:p-8 shadow-[0_2px_20px_rgba(0,0,0,0.03)] border border-gray-100 relative">
        ...
        <div className="flex flex-col sm:flex-row bg-[#F3F4F6] p-1.5 rounded-xl border border-gray-100 mb-8 relative z-10 gap-1">
          <label className={`flex-1 text-center py-3 rounded-lg text-xs font-bold uppercase tracking-wider cursor-pointer transition-all duration-300 ${emailConfig.provider === "google_oauth" ? "bg-white text-[var(--primary,#9333EA)] shadow-sm" : "text-gray-500 hover:text-gray-900"}`}>
            <input type="radio" name="provider" value="google_oauth" checked={emailConfig.provider === "google_oauth"} onChange={() => setEmailConfig({...emailConfig, provider: "google_oauth"})} className="hidden" />
            Google
          </label>
          <label className={`flex-1 text-center py-3 rounded-lg text-xs font-bold uppercase tracking-wider cursor-pointer transition-all duration-300 ${emailConfig.provider === "microsoft" ? "bg-white text-[var(--primary,#9333EA)] shadow-sm" : "text-gray-500 hover:text-gray-900"}`}>
            <input type="radio" name="provider" value="microsoft" checked={emailConfig.provider === "microsoft"} onChange={() => setEmailConfig({...emailConfig, provider: "microsoft"})} className="hidden" />
            Microsoft
          </label>
          <label className={`flex-1 text-center py-3 rounded-lg text-xs font-bold uppercase tracking-wider cursor-pointer transition-all duration-300 ${emailConfig.provider === "custom" ? "bg-white text-[var(--primary,#9333EA)] shadow-sm" : "text-gray-500 hover:text-gray-900"}`}>
        ...
        <div className="space-y-6 relative z-10">
          ...
          {emailConfig.provider === "google_oauth" && (
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                {!isGoogleConnected && (
                  <div className="flex-1">
                    {savedProvider && savedProvider !== "google_oauth" && (
                      <p className="text-[11px] font-medium text-amber-600 bg-amber-50 p-3 rounded-xl border border-amber-100 mb-3 leading-relaxed">
                        <strong>Safe Switch:</strong> You are currently using {activeProviderName}. If this Google login is cancelled or fails, your {activeProviderName} connection will remain perfectly intact.
                      </p>
                    )}
                    <button onClick={() => openOAuthPopup('google')} className="w-full inline-flex items-center justify-center gap-3 py-3.5 px-4 bg-white hover:bg-gray-50 text-gray-800 text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-sm border border-gray-200">
                      ...
                      {savedProvider && savedProvider !== "google_oauth" ? "Switch to Google" : "Sign in with Google"}
                    </button>
                  </div>
                )}
          ...
          {emailConfig.provider === "microsoft" && (
            <div className="p-6 bg-[#F8FAFC] border border-gray-100 rounded-2xl space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                {!isMicrosoftConnected && (
                  <div className="flex-1">
                    {savedProvider && savedProvider !== "microsoft" && (
                      <p className="text-[11px] font-medium text-amber-600 bg-amber-50 p-3 rounded-xl border border-amber-100 mb-3 leading-relaxed">
                        <strong>Safe Switch:</strong> You are currently using {activeProviderName}. If this Microsoft login is cancelled or fails, your {activeProviderName} connection will remain perfectly intact.
                      </p>
                    )}
                    <button onClick={() => openOAuthPopup('microsoft')} className="w-full inline-flex items-center justify-center gap-3 py-3.5 px-4 bg-[#2F2F2F] hover:bg-black text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-sm">
                      ...
                      {savedProvider && savedProvider !== "microsoft" ? "Switch to Microsoft" : "Sign in with Microsoft"}
                    </button>
                  </div>
                )}
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" value={emailConfig.smtpHost} onChange={(e) => setEmailConfig({...emailConfig, smtpHost: e.target.value})} className="w-full bg-[#F3F4F6] rounded-xl px-4 py-3.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-[var(--primary,#8B5CF6)] focus:ring-opacity-30 transition-all font-mono border-none" placeholder="smtp.mailserver.com" />
                </div>
                <div>
