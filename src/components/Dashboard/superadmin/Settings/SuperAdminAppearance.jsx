...
const applyThemeToDOM = (mode, hue, shade) => {
  // Default to light if nothing is provided
...
// Helper function to read local storage synchronously on mount
const getInitialSettings = () => {
  if (typeof window === "undefined") return null;
  try {
    const saved = localStorage.getItem("aicyro_appearance_superadmin");
...
export default function SuperAdminAppearance() {
  // --- Initialize state synchronously from Local Storage to prevent FOUC, defaulting to "light"
  const [activeMode, setActiveMode] = useState(
    () => getInitialSettings()?.mode || "light",
  );
  const [activeHue, setActiveHue] = useState(
    () => getInitialSettings()?.hue || "default",
  );
  const [accentShade, setAccentShade] = useState(
    () => getInitialSettings()?.shade || 500,
  );
  ...
  const [activePreview, setActivePreview] = useState("clients");

  // Fetch Super Admin User
  useEffect(() => {
    const storedUser = localStorage.getItem("currentSuperAdmin");
    if (storedUser) {
      setUsername(storedUser);
    }
  }, []);

  // Fetch saved theme from Firebase on mount
  useEffect(() => {
    const loadPreferencesFromDB = async () => {
      const activeUser = username || localStorage.getItem("currentSuperAdmin");
  ...
  // Save selected theme back to Firebase
  const savePreferencesToDB = async () => {
    const activeUser = username || localStorage.getItem("currentSuperAdmin");
    if (!activeUser) {
      showToast("Error: No admin logged in.");
      return;
    }
  ...
  // Apply theme dynamically as user tweaks controls
  useEffect(() => {
    applyThemeToDOM(activeMode, activeHue, accentShade);
  }, [activeMode, activeHue, accentShade]);

  // On unmount or initial load, ensure the active theme matches saved preference
  useEffect(() => {
    return () => {
  ...
  return (
    <div className="flex bg-[var(--background)] text-[var(--foreground)] font-sans transition-colors duration-300">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-8 border-b border-[var(--border-color)]">
          <div>
            ...
            <p className="text-sm text-[var(--foreground-muted)] mt-2">
              Customize your console mode and accent color.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 mt-8">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <section className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-6 shadow-sm transition-colors duration-300">
              ...
              {/* Changed to 2 columns since System is removed */}
              <div className="grid grid-cols-2 gap-2 bg-[var(--background)] border border-[var(--border-color)] rounded-xl p-1.5">
                {[
                  {
                    id: "light",
                    label: "Light",
          ...
          <section className="lg:col-span-8 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl shadow-sm overflow-hidden flex flex-col transition-colors duration-300 h-fit sticky top-8">
            ...
            <div className="bg-[var(--card-bg)] border-b border-[var(--border-color)] px-4 flex gap-6 text-xs font-bold z-20 relative">
              <button
                onClick={() => setActivePreview("clients")}
                className={`py-3 border-b-2 transition-colors uppercase tracking-widest ${activePreview === "clients" ? "border-[var(--primary)] text-[var(--primary)]" : "border-transparent text-[var(--foreground-muted)] hover:text-[var(--foreground)]"}`}
              >
                Clients
              </button>
              <button
                onClick={() => setActivePreview("billing")}
                className={`py-3 border-b-2 transition-colors uppercase tracking-widest ${activePreview === "billing" ? "border-[var(--primary)] text-[var(--primary)]" : "border-transparent text-[var(--foreground-muted)] hover:text-[var(--foreground)]"}`}
              >
                Billing
              </button>
            </div>

            <div className="relative w-full h-[550px] md:h-[700px] bg-[var(--background)] overflow-hidden">
              <div
