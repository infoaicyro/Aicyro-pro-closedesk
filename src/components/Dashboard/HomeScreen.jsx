...
export default function HomeScreen({ onLogout }) {
  ...
  // Interactive Dashboard Controls - Default is 'All'
  ...
  useEffect(() => {
    // 1. Listen for LIVE Data (Visitors)
    const visitorsRef = ref(db, "analytics/visitors");
          ...
          const liveData = Object.keys(data)
            .map((key) => {
              ...
              return {
                ...
                deviceName: profile.deviceName || "Unknown Device",
                lead_score: visitor.lead_score || autoScore,
                booking_status: visitor.booking_status || "Captured",
                email: visitor.email || "",
                phone: visitor.phone || "",
              };
            })
    ...
    // 2. Listen for Actual Leads (For 100% Accurate Booking KPI)
    const leadsDataRef = ref(db, "leads");
    const unsubscribeBookings = onValue(
      leadsDataRef,
      (snapshot) => {
        if (snapshot.exists()) {
          setBookingsData(Object.values(snapshot.val()));
        } else {
          setBookingsData([]);
        }
      },
    ...
    // 3. Listen for Analytics Summary
    ...
    // 4. Listen for Chatbot Config Data
    ...
    // 5. Listen for Business Profile Data
  ...
  const filteredVisitors = useMemo(() => {
    ...
    return leads.filter((visitor) => {
      if (!visitor.timestamp) return false;
      const visitorTime = new Date(visitor.timestamp).getTime();
      return visitorTime >= todayTime - timeFrame * ONE_DAY;
    });
  }, [leads, timeFrame]);

  // 2. Filter Leads (For accurate Bookings count)
  const filteredBookingsData = useMemo(() => {
    if (timeFrame === "All") return bookingsData;
    ...
    return bookingsData.filter((b) => {
      const t =
        b.timestamp || b.conversation_started_at || b.conversation_ended_at;
      if (!t) return false;
      return new Date(t).getTime() >= todayTime - timeFrame * ONE_DAY;
    });
  }, [bookingsData, timeFrame]);

  // Base metrics
  const scaledVisitors =
    timeFrame === "All" ? totalWebsiteVisitors : filteredVisitors.length;
  const scaledOpens =
    timeFrame === "All"
      ? totalChatbotOpens
      : filteredVisitors.reduce(
          (sum, v) => sum + (v.summary?.chat_opens_count || 0),
          0,
        );
  const scaledConvos =
    timeFrame === "All"
      ...
      : filteredVisitors.reduce(
          (sum, v) => sum + (v.summary?.conversations_count || 0),
          0,
        );

  // Captured Leads array (From Visitors)
  const capturedLeadsArr = filteredVisitors.filter(
    (v) =>
      v.summary?.conversations_count > 0 ||
      v.email ||
      v.phone ||
      v.lead_score === "High" ||
      v.lead_score === "Medium",
  ...
  // --- FUNNEL LOGIC & FAILSAFE METRICS ---

  // Perfectly synced calculation for Booked Appointments
  const bookedAppointments = filteredBookingsData.filter((l) => {
    const status = (l.booking_status || "").toLowerCase().trim();

    // Check if it has a valid date.
    let hasValidDate = false;
    if (l.booked_slot) {
      hasValidDate = !isNaN(new Date(l.booked_slot).getTime());
    } else if (
      l.preferred_date &&
      ...
      hasValidDate = !isNaN(
        new Date(`${l.preferred_date} ${l.preferred_time}`).getTime(),
      );
    }

    // Check if it has a terminal status explicitly
    const isTerminal =
      status === "completed" ||
      status === "cancelled" ||
      status === "no-show" ||
    ...
    return hasValidDate || isTerminal;
  }).length;

  // Protect against database fragmentation across nodes
  let leadsCaptured = capturedLeadsArr.length;
  if (bookedAppointments > leadsCaptured) leadsCaptured = bookedAppointments;

  let qualifiedLeads = capturedLeadsArr.filter(
    (l) => l.lead_score === "High" || l.lead_score === "Medium",
  ).length;
  if (bookedAppointments > qualifiedLeads) qualifiedLeads = bookedAppointments;

  const afterHoursLeads = capturedLeadsArr.filter(
  ...
  // Waterfall logic to ensure Donut Chart totals 100% perfectly
  ...
  // --- CHART CALCULATIONS ---
  const highLeads = capturedLeadsArr.filter(
    (l) => l.lead_score === "High",
  ).length;
  const mediumLeads = capturedLeadsArr.filter(
  ...
  const chartData = useMemo(() => {
    ...
    filteredVisitors.forEach((visitor) => {
      if (!visitor.timestamp) return;
      let vDate = new Date(visitor.timestamp);
      vDate.setHours(0, 0, 0, 0);
      const vTime = vDate.getTime();

      const targetDay = structure.find((d) => d.dateValue === vTime);
      if (targetDay) {
        targetDay.visitors += 1;
        targetDay.convos += visitor.summary?.conversations_count || 0;

        const isCaptured =
          visitor.summary?.conversations_count > 0 ||
          visitor.email ||
          visitor.phone ||
          visitor.lead_score === "High" ||
          visitor.lead_score === "Medium";
        if (isCaptured) {
          targetDay.leads += 1;
        }
      }
    ...
    return structure;
  }, [filteredVisitors, effectiveTimeFrame]);

  const svgWidth = 800;
  const svgHeight = 240;

  const maxDataValue = Math.max(
    ...chartData.map((d) => Math.max(d.visitors, d.convos, d.leads)),
    10,
  );
        ...
        <div className="xl:col-span-8 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-4 sm:p-8 shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="flex-grow relative h-[220px] sm:h-[300px] w-full mt-2 sm:mt-4">
            <svg
              ...
              className="absolute inset-0 w-full h-full overflow-hidden pb-6 pointer-events-none"
            >
              {animateCharts && (
                <>
                  <path
